...
import { PieChart, Pie, Cell, BarChart, Bar, LineChart as RechartsLineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer } from 'recharts'
...
function Dashboard({ token, onLogout, onTokenRefresh }) {
  ...
  const fetchDashboard = useCallback(async () => {
    try {
      ...
      const [sumRes, expRes, prevExpRes, catRes, budRes, incRes, trendRes, biRes] = await Promise.all([
        ...
        fetch(`http://localhost:5000/api/dashboard/bi/${month}`, { headers })
      ])
      const [sumData, expData, prevExpData, catData, budData, incData, trendData, biDataResult] = await Promise.all([
        sumRes.json(), expRes.json(), prevExpRes.json(), catRes.json(), budRes.json(), incRes.json(), trendRes.json(), biRes.json()
      ])
      ...
      if (biDataResult.success) setBiData(biDataResult.data)
    } catch (err) {
      console.error(err)
    }
  ...
  const biSummary = biData?.summary || {
  ...
  const biCategories = biData?.categories?.length ? biData.categories : pieData.map((item) => ({
    category_name: item.name,
    total_spent: item.value,
    spend_percentage: Number(summary?.total_expense || 0) > 0 ? (item.value / Number(summary?.total_expense || 0)) * 100 : 0
  }))

  const biBudgets = biData?.budgets?.length ? biData.budgets : budgetProgress.map((item) => ({
    category_name: item.category_name,
    budget_amount: item.budgetAmount,
    total_spent: item.spent,
    remaining_budget: item.remaining,
    budget_status: item.overBudget ? 'Exceeded' : item.percentage >= 80 ? 'Near Limit' : 'OK'
  }))

  const biTrends = biData?.trends?.length ? biData.trends : trends
  const topBiCategory = biCategories[0]
  const overBudgetItems = biBudgets.filter((item) => item.budget_status === 'Exceeded')
  const nearLimitItems = biBudgets.filter((item) => item.budget_status === 'Near Limit')
  const savingsRate = Number(biSummary.savings_rate || 0)
  const expenseRatio = Number(biSummary.expense_ratio || 0)
  const biTopExpenses = biData?.topExpenses?.length ? biData.topExpenses : expenses.slice().sort((a, b) => Number(b.amount) - Number(a.amount)).slice(0, 5)
  const biAlerts = biData?.alerts || []
    ...
    <div className={`dashboard-layout${isLowBalance ? ' low-balance-warning' : ''}`}>
        ...
        <div className="db-mobile-overlay" onClick={() => setDbMenuOpen(false)}>
          <div className="db-mobile-drawer" onClick={(e) => e.stopPropagation()}>
            ...
            <nav className="db-mobile-nav">
              ...
              <button className={`sidebar-link ${activeTab === 'bi' ? 'active' : ''}`} onClick={() => { setActiveTab('bi'); setDbMenuOpen(false); }}><BarChart3 size={20} /> <span>BI Dashboard</span></button>
      ...
      <aside className="dashboard-sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <button className={`sidebar-link ${activeTab === 'bi' ? 'active' : ''}`} onClick={() => setActiveTab('bi')}><BarChart3 size={20} /> <span>BI Dashboard</span></button>
      ...
      <main className="dashboard-main">
        ...
        <div className="dashboard-header dashboard-header-card">
          <div>
            <h2>{activeTab === 'overview' ? 'Financial Command Center' : activeTab === 'bi' ? 'Business Intelligence Dashboard' : activeTab === 'transactions' ? 'Transaction Studio' : activeTab === 'analysis' ? 'Category Analysis' : activeTab === 'budgets' ? 'Budget Setup Lab' : activeTab === 'targets' ? 'Budget Targets & Roadmap' : activeTab === 'profile' ? 'Account Profile' : 'Income Control Room'}</h2>
            <p>
              {activeTab === 'overview' ? `Advanced financial overview for ${month}` : activeTab === 'bi' ? `BI report with KPIs, trends, category shares, budgets, and alerts for ${month}` : activeTab === 'transactions' ? `All transactions for ${month}` : activeTab === 'analysis' ? `Compare category spending between ${previousMonth} and ${month}` : activeTab === 'budgets' ? `Set category budgets and AI plans for ${month}` : activeTab === 'targets' ? `Visual roadmap and remaining balance for ${month}` : activeTab === 'profile' ? 'Update your account details, password, and account status.' : `Income and salary insights for ${month}`}
            </p>
          </div>
          ...
          <div className="bi-dashboard">
            <div className="bi-kpi-grid">
              <article className="bi-kpi-card">
                ...
                <strong>{formatMoney(biSummary.total_income || 0)}</strong>
                <small>Money available in {month}</small>
              </article>
              <article className="bi-kpi-card danger">
                <span>Total Expenses</span>
                <strong>{formatMoney(biSummary.total_expense || 0)}</strong>
                <small>{expenseRatio.toFixed(1)}% of income used</small>
              </article>
              <article className="bi-kpi-card success">
                <span>Savings</span>
                <strong>{formatMoney(biSummary.remaining_balance || 0)}</strong>
                <small>{savingsRate.toFixed(1)}% savings rate</small>
              </article>
              <article className="bi-kpi-card warning">
                <span>Top Category</span>
                <strong>{topBiCategory?.category_name || 'No data'}</strong>
                <small>{topBiCategory ? formatMoney(topBiCategory.total_spent) : 'Add expenses to analyze'}</small>
              </article>
            </div>

            <div className="bi-grid">
              <section className="panel bi-chart-panel">
                <div className="analysis-panel-head">
                  <div>
                    <h3>Category Wise Expense Analysis</h3>
                    <p>Shows which category takes the biggest share of spending.</p>
                  </div>
                  <span className="analysis-chip">{biCategories.length} categories</span>
                </div>
                {biCategories.length > 0 ? (
                  <ResponsiveContainer width="100%" height={320}>
                    <PieChart>
                      <Pie data={biCategories} cx="50%" cy="50%" innerRadius={66} outerRadius={104} paddingAngle={4} dataKey="total_spent" nameKey="category_name">
                        {biCategories.map((entry, index) => <Cell key={`bi-cell-${entry.category_name}`} fill={COLORS[index % COLORS.length]} />)}
                      </Pie>
                      <RechartsTooltip formatter={(value, name, item) => [formatMoney(value), item?.payload?.category_name || name]} contentStyle={{ backgroundColor: 'rgba(2,6,12,0.94)', border: '1px solid var(--border-light)', borderRadius: '10px' }} />
                    </PieChart>
                  </ResponsiveContainer>
                ) : (
                  <p className="analysis-empty">No expense data for this month.</p>
                )}
              </section>
              ...
              <section className="panel bi-chart-panel">
                <div className="analysis-panel-head">
                  <div>
                    <h3>Expense Trend</h3>
                    <p>Income, expenses, and balance across available months.</p>
                  </div>
                </div>
                {biTrends.length > 0 ? (
                  <ResponsiveContainer width="100%" height={320}>
                    <RechartsLineChart data={biTrends} margin={{ top: 16, right: 20, left: 8, bottom: 8 }}>
                      ...
                      <RechartsTooltip formatter={(value) => formatMoney(value)} contentStyle={{ backgroundColor: 'rgba(2,6,12,0.94)', border: '1px solid var(--border-light)', borderRadius: '10px' }} />
                      <Line type="monotone" dataKey="total_income" name="Income" stroke="#22c55e" strokeWidth={3} dot={{ r: 4 }} />
                      <Line type="monotone" dataKey="total_expense" name="Expenses" stroke="#ef4444" strokeWidth={3} dot={{ r: 4 }} />
                      <Line type="monotone" dataKey="remaining_balance" name="Savings" stroke="#38bdf8" strokeWidth={3} dot={{ r: 4 }} />
                    </RechartsLineChart>
                  </ResponsiveContainer>
                ) : (
                  <p className="analysis-empty">No trend data available yet.</p>
                )}
              </section>

              <section className="panel bi-chart-panel bi-wide">
                <div className="analysis-panel-head">
                  <div>
                    <h3>Budget vs Actual Spending</h3>
                    <p>Compares planned budget amounts with real spending.</p>
                  </div>
                  <span className="analysis-chip">{overBudgetItems.length} over budget</span>
                </div>
                {biBudgets.length > 0 ? (
                  <ResponsiveContainer width="100%" height={330}>
                    <BarChart data={biBudgets} margin={{ top: 20, right: 20, left: 10, bottom: 60 }}>
                      <XAxis dataKey="category_name" stroke="var(--text-dim)" angle={-24} textAnchor="end" interval={0} height={78} />
                      <YAxis stroke="var(--text-dim)" tickFormatter={(value) => `${Math.round(value / 1000)}k`} />
                      <RechartsTooltip formatter={(value) => formatMoney(value)} cursor={{ fill: 'rgba(255,255,255,0.05)' }} contentStyle={{ backgroundColor: 'rgba(2,6,12,0.94)', border: '1px solid var(--border-light)', borderRadius: '10px' }} />
                      <Bar dataKey="budget_amount" name="Budget" fill="#38bdf8" radius={[8, 8, 0, 0]} />
                      <Bar dataKey="total_spent" name="Actual" fill="#22c55e" radius={[8, 8, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <p className="analysis-empty">Set category budgets to unlock budget BI.</p>
                )}
              </section>
            </div>
            ...
            <div className="bi-lower-grid">
              <section className="panel">
                <div className="analysis-panel-head">
                  <div>
                    ...
                    <p>Largest individual transactions in {month}.</p>
                  </div>
                </div>
                <div className="bi-list">
                  {biTopExpenses.length === 0 && <p className="analysis-empty">No expenses to rank.</p>}
                  {biTopExpenses.map((expense) => (
                    <article key={expense.expense_id} className="bi-list-row">
                      <div>
                        <strong>{expense.category_name}</strong>
                        <span>{expense.note || 'No note'} | {new Date(expense.expense_date).toLocaleDateString()}</span>
                      </div>
              ...
              <section className="panel">
                <div className="analysis-panel-head">
                  <div>
                    <h3>BI Insights</h3>
                    <p>Simple decision points from your data.</p>
                  </div>
                </div>
                <div className="bi-insight-stack">
                  <article>
                    <span>Budget Health</span>
                    <strong>{overBudgetItems.length === 0 ? 'Healthy' : `${overBudgetItems.length} categories exceeded`}</strong>
                    <p>{nearLimitItems.length} categories are near the limit.</p>
                  </article>
                  <article>
                    <span>Cash Flow Status</span>
                    <strong>{Number(biSummary.remaining_balance || 0) >= 0 ? 'Positive' : 'Overspending'}</strong>
                    <p>{expenseRatio.toFixed(1)}% expense-to-income ratio.</p>
                  </article>
                  <article>
                    <span>Alerts</span>
                    <strong>{biAlerts.length}</strong>
                    <p>{biAlerts.length ? biAlerts[0].alert_message : 'No budget alerts for this month.'}</p>
                  </article>
                </div>
              </section>
            </div>
